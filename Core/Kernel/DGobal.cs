...
// Type: zgcSpaceKernel.Core.DGobal
...
namespace zgcSpaceKernel.Core
{
    public class DGobal
    {
        ...
        //        case "AppCoreDb":
        //            return DGobal.DbName.AppCoreDb;
        //        case "CustomerDb":
        ...
        {
            if (DGobal._dD_DBStr == null)
                DGobal.Init();
            C[] cArray = DGobal._dD_DBStr[key];
        ...
        public static object[] Dict(string key)
        {
            if (DGobal._dD_List_fd_a == null)
                DGobal.Load_fda(key);
            return DGobal._dD_List_fd_a[key];
        }
        ...
        public static void Load_fda(string ModelDb)
        {
          ...
          "gcGOBAL_PROC_RunSQL"
            ...
            x4.Init(DGobal.SqlString(DGobal.gbDatabaseDb, true), (Dictionary<string, C>)null, DGobal.gbDatabaseDb);
            x4.jSonFormat = true;
            x4.GD(sql4);
        ...
        {
            if (DGobal._dD_List_fd_a == null)
                DGobal.Load_fda(DbName);
            if (DGobal._dD_List_fd_a.ContainsKey(DbName))
                return DGobal._dD_List_fd_a[DbName];
            DGobal.Load_fda(DbName);
            return DGobal._dD_List_fd_a.ContainsKey(DbName) ? DGobal._dD_List_fd_a[DbName] : (object[])null;
        }
        ...
        public static C Lookup_aByKey(string DbName, string action)
        {
            ...
            if (DGobal._dD_List_fd_a.ContainsKey(DbName))
            {
                Dictionary<string, C> dictionary = DGobal._dD_List_fd_a[DbName][0] as Dictionary<string, C>;
        ...
        public static Dictionary<int, C[]> _fd(string DbName)
        {
            if (DGobal._dD_List_fd_a == null)
                DGobal.Load_fda(DbName);
            if (DGobal._dD_List_fd_a.ContainsKey(DbName))
                return DGobal._dD_List_fd_a[DbName][1] as Dictionary<int, C[]>;
            DGobal.Load_fda(DbName);
            return DGobal._dD_List_fd_a.ContainsKey(DbName) ? DGobal._dD_List_fd_a[DbName][1] as Dictionary<int, C[]> : (Dictionary<int, C[]>)null;
        }
        ...
        new C() { T = new string[3]{ "Keep Space", "Not Use", "" } }
            ...
            DGobal._dD_DBStr = dictionary;
            string sql = "SELECT * FROM table_databases ";
            X x = new X((object)null);
            x.Init(DGobal.SqlString(DGobal.gbDatabaseDb, true), (Dictionary<string, C>)null, DGobal.gbDatabaseDb);
            x.jSonFormat = true;
            x.GD(sql);
          ...
          new C()
          {
            T = new string[5]
            {
              ...
              str
                ...
                if (DGobal._dD_DBStr == null)
                    DGobal._dD_DBStr = new Dictionary<string, C[]>();
                DGobal._dD_DBStr.Add(key, cArray);
            }
        }
