  ...
  public class X
  {
    ...
    public string LW(List<C> f)
    {
      ...
      for (int index = 0; index < f.Count; ++index)
      {
        ...
        string T = c.T[2].Trim();
        if (str1 != "")
          this.FindOp(c.T[2], out T, out op);
        string str2 = T.IndexOf("$x") == -1 ? c.T[0] + "=" + (c.T[1][0] == '0' || c.T[1][0] == '1' ? T : "N'" + T + "'") : T.Replace("$x", c.T[0]);
        if (str1 == "")
    ...
    public void IoU(string sql)
    {
      ...
      using (SqlConnection sqlConnection = new SqlConnection(this._cs))
      {
        ...
        try
        {
          ...
          this._r._d = command.ExecuteScalar();
        }
        catch (Exception ex)
        {
          this._r._s = 0;
    ...
    public void I(string v, List<C> d)
    {
      ...
      {
        this._sql += "; select SCOPE_IDENTITY()";
